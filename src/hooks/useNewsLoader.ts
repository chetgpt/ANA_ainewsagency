...
export function useNewsLoader() {
  ...
  const [preloadedNews, setPreloadedNews] = useState<NewsScript[]>([]);
  ...
  const preloadNextBatchOfNews = async () => {
    ...
    try {
      ...
      const validArticles = processedArticles.filter(article => article !== null) as NewsScript[];
  ...
  // Load the next news article from preloaded queue or fetch new one
